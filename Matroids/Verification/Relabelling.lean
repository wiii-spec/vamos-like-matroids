...
lemma sort_of_length [LinearOrder X] {A : List X}:
  ...
  unfold List.sort
  exact List.Perm.symm (List.mergeSort_perm A (fun x x_1 => x ≤ x_1))
...
lemma flatten_of_relabelling {L : List (List Nat)} {f : ℕ → ℕ} :
  List.flatten (relabelling L f) = List.map f (List.flatten L) := by
  unfold relabelling
  simp
